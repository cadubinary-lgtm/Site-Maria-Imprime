...
export function MariaGuide({ content, compact = false }: { content?: MariaGuideContent; compact?: boolean }) {
  ...
  return <section className={compactGuideClass} aria-labelledby="maria-guide-title">
    ...
    <footer className="mt-4 flex flex-col gap-4 rounded-xl border border-pink-200 bg-white px-4 py-4 lg:grid lg:grid-cols-[minmax(0,1fr)_9.5rem] lg:items-center" style={{ paddingTop: "3px" }}><div className="flex min-w-0 items-start gap-3"><CircleHelp className="mt-0.5 h-5 w-5 shrink-0 text-pink-600" /><div><p className="font-extrabold text-slate-950">{guide.bottomNoteTitle}</p><p className="mt-1 text-xs leading-5 text-slate-600">{guide.bottomNote}</p></div></div><div className="flex w-full flex-row gap-2 lg:w-[9.5rem]"><a href="/documentos" className={`${HOME_SECONDARY_ACTION_CLASS} min-w-0 flex-1 px-3 text-center text-xs leading-tight`}><FileText className="h-4 w-4 shrink-0" />Ver normas para envio de arte</a><a href="#maria-guide-entrega" className={`${HOME_PRIMARY_ACTION_CLASS} lg:hidden min-w-0 flex-1 px-3 text-center text-xs leading-tight`}><Truck className="h-4 w-4 shrink-0" />Falar com a Maria</a></div></footer>
  </section>;
}
