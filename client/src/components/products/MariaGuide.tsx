...
export function MariaGuide({ content, compact = false }: { content?: MariaGuideContent; compact?: boolean }) {
  ...
  const compactGuideClass = compact
    ? "mt-3 rounded-xl border border-pink-100 bg-white p-2 shadow-[0_10px_28px_rgba(236,17,116,0.05)] sm:p-3 [&_.mg-top-card]:min-h-[52px] [&_.mg-top-card]:overflow-hidden [&_.mg-top-card]:p-1.5 [&_.mg-top-content]:gap-1.5 [&_.mg-top-icon]:p-1 [&_.mg-top-icon_svg]:h-3.5 [&_.mg-top-icon_svg]:w-3.5 [&_.mg-top-title]:text-[11px] [&_.mg-top-subtitle]:mt-0 [&_.mg-top-subtitle]:line-clamp-2 [&_.mg-top-subtitle]:text-[9px] [&_.mg-top-subtitle]:leading-2.5 [&_.mg-panel]:min-h-[330px] [&_.mg-panel-header]:gap-2 [&_.mg-panel-header]:px-3 [&_.mg-panel-header]:py-3 [&_.mg-panel-header_svg]:h-5 [&_.mg-panel-header_svg]:w-5"
  ...
  return <section className={compactGuideClass} aria-labelledby="maria-guide-title">
    ...
    <div className={topGridClass}>{panels.map((section) => { const isExpanded = expandedPanels.has(section.id); const hasPinkBackground = isExpanded; return <button key={section.id} type="button" onClick={() => togglePanel(section)} className={`mg-top-card min-h-[104px] rounded-xl border p-4 text-left transition ${hasPinkBackground ? "border-pink-500 bg-pink-50 shadow-sm" : "border-slate-200 bg-white hover:border-pink-200 hover:bg-pink-50/30"}`} aria-expanded={isExpanded} aria-controls={`maria-guide-${section.id}`}><div className="mg-top-content flex h-full items-start gap-4"><div className="mg-top-icon rounded-lg bg-pink-50 p-2 text-pink-600"><GuideIcon name={section.icon} className="h-8 w-8" /></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><h3 className="mg-top-title font-extrabold text-slate-950">{section.title}</h3><ChevronDown className={`h-4 w-4 text-pink-600 transition-transform ${isExpanded ? "rotate-180" : ""}`} /></div><p className="mg-top-subtitle mt-1 text-xs leading-5 text-slate-600">{section.subtitle}</p></div></div></button>; })}</div>
