...
export function MariaGuide({ content }: { content?: MariaGuideContent }) {
  ...
  const focusPanel = (section: MariaGuideSection) => { setHighlightedId(section.id); document.getElementById(`maria-guide-${section.id}`)?.scrollIntoView({ behavior: "smooth", block: "center" }); };
  ...
  return <section className="mt-8 rounded-2xl border border-pink-100 bg-white p-3 shadow-[0_14px_40px_rgba(236,17,116,0.06)] sm:p-5" aria-labelledby="maria-guide-title">
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{panels.map((section) => { const isHighlighted = section.id === highlightedId; return <button key={section.id} type="button" onClick={() => focusPanel(section)} className={`rounded-xl border p-4 text-left transition ${isHighlighted ? "border-pink-500 bg-pink-50 shadow-sm" : "border-slate-200 bg-white hover:border-pink-200 hover:bg-pink-50/30"}`} aria-pressed={isHighlighted}><div className="flex items-start gap-4"><div className="rounded-lg bg-pink-50 p-2 text-pink-600"><GuideIcon name={section.icon} className="h-8 w-8" /></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><h3 className="font-extrabold text-slate-950">{section.title}</h3><ChevronDown className="h-4 w-4 text-pink-600" /></div><p className="mt-1 text-xs leading-5 text-slate-600">{section.subtitle}</p></div></div></button>; })}</div>
