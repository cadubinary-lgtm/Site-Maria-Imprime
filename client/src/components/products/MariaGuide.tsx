...
export function MariaGuide({ content, compact = false }: { content?: MariaGuideContent; compact?: boolean }) {
  ...
  const finishRows = Array.from({ length: Math.ceil(finishItems.length / 3) }, (_, rowIndex) => finishItems.slice(rowIndex * 3, rowIndex * 3 + 3));
  ...
  const compactGuideClass = compact
    ? "mt-3 rounded-xl border border-pink-100 bg-white p-2 shadow-[0_10px_28px_rgba(236,17,116,0.05)] sm:p-3 [&_.mg-top-card]:min-h-[40px] [&_.mg-top-card]:overflow-hidden [&_.mg-top-card]:p-1 [&_.mg-top-content]:gap-1 [&_.mg-top-icon]:rounded-md [&_.mg-top-icon]:p-0.5 [&_.mg-top-icon_svg]:h-3 [&_.mg-top-icon_svg]:w-3 [&_.mg-top-title]:text-[10px] [&_.mg-top-subtitle]:hidden [&_.mg-panel]:min-h-[330px] [&_.mg-panel-header]:gap-2 [&_.mg-panel-header]:px-3 [&_.mg-panel-header]:py-3 [&_.mg-panel-header_svg]:h-5 [&_.mg-panel-header_svg]:w-5"
    : "mt-8 rounded-2xl border border-pink-100 bg-white p-3 shadow-[0_14px_40px_rgba(236,17,116,0.06)] sm:p-5";
  const topGridClass = compact ? "grid items-stretch gap-2 grid-cols-2 md:grid-cols-4" : "grid items-stretch gap-3 md:grid-cols-2 xl:grid-cols-4";
  const responsivePanelGridClass = compact
    ? expandedPanelCount >= 4 ? "md:grid-cols-4" : expandedPanelCount === 3 ? "md:grid-cols-3" : expandedPanelCount === 2 ? "md:grid-cols-2" : "md:grid-cols-1"
    : panelGridClass;
  const panelSpacingClass = compact ? "mt-1.5 gap-1.5" : "mt-3 gap-2";

  if (panels.length !== 4) return null;

  return <section className={compactGuideClass} aria-labelledby="maria-guide-title">
    <div className="sr-only"><h2 id="maria-guide-title">{guide.title}</h2><p>{guide.description}</p></div>

    <div className={topGridClass}>{panels.map((section) => { const isExpanded = expandedPanels.has(section.id); const hasPinkBackground = isExpanded; return <button key={section.id} type="button" onClick={() => togglePanel(section)} className={`mg-top-card min-h-[104px] rounded-xl border p-4 text-left transition ${hasPinkBackground ? "border-pink-500 bg-pink-50 shadow-sm" : "border-slate-200 bg-white hover:border-pink-200 hover:bg-pink-50/30"}`} aria-expanded={isExpanded} aria-controls={`maria-guide-${section.id}`}><div className={`mg-top-content flex h-full ${compact ? "items-center gap-1" : "items-start gap-4"}`}><div className="mg-top-icon rounded-lg bg-pink-50 p-2 text-pink-600"><GuideIcon name={section.icon} className="h-8 w-8" /></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><h3 className="mg-top-title font-extrabold text-slate-950">{section.title}</h3><ChevronDown className={`${compact ? "h-3 w-3" : "h-4 w-4"} text-pink-600 transition-transform ${isExpanded ? "rotate-180" : ""}`} /></div>{!compact && <p className="mg-top-subtitle mt-1 text-xs leading-5 text-slate-600">{section.subtitle}</p>}</div></div></button>; })}</div>

    <div className={`${panelSpacingClass} grid ${responsivePanelGridClass} ${!expandedPanels.has("impressao") ? "[&>#maria-guide-impressao]:hidden" : ""} ${!expandedPanels.has("material") ? "[&>#maria-guide-material]:hidden" : ""} ${!expandedPanels.has("acabamento") ? "[&>#maria-guide-acabamento]:hidden" : ""} ${!expandedPanels.has("entrega") ? "[&>#maria-guide-entrega]:hidden" : ""}`}>
      ...
      <article id="maria-guide-impressao" className={`mg-panel flex min-h-[450px] flex-col overflow-hidden rounded-xl border bg-white transition-shadow ${highlightedId === "impressao" ? "border-pink-300 shadow-md" : "border-slate-200"}`}><header className="mg-panel-header flex items-start gap-3 px-4 py-4"><div className="rounded-lg bg-pink-50 p-2 text-pink-600"><GuideIcon name="printer" className="h-7 w-7" /></div><div><h3 className="font-extrabold text-slate-950">Tipos de impressão</h3><p className="mt-1 text-xs leading-5 text-slate-600">{printSection?.description}</p></div></header><div className="border-y border-slate-100">{printingItems.map((item) => <ItemRow key={item.id} item={item} icon="printer" isOpen={openPrintId === item.id} onClick={() => setOpenPrintId(openPrintId === item.id ? null : item.id)} />)}</div><GuideTip text={printSection?.tip ?? "Em caso de dúvida, nossa equipe pode ajudar a encontrar a melhor tecnologia."} /></article>

      <article id="maria-guide-material" className={`mg-panel flex min-h-[450px] flex-col overflow-hidden rounded-xl border bg-white transition-shadow ${highlightedId === "material" ? "border-pink-300 shadow-md" : "border-slate-200"}`}>
        <header className="mg-panel-header flex items-start gap-3 px-4 py-4"><div className="rounded-lg bg-pink-50 p-2 text-pink-600"><GuideIcon name="layers" className="h-7 w-7" /></div><div><h3 className="font-extrabold text-slate-950">Materiais disponíveis</h3><p className="mt-1 text-xs leading-5 text-slate-600">{materialSection?.description}</p></div></header>
        <div className="border-y border-slate-100">
          {materialCategories.map((category) => <div key={category.id} className="border-b border-slate-100 last:border-b-0">
            <button type="button" onClick={() => setOpenMaterialId(openMaterialId === category.id ? null : category.id)} className={`w-full px-3 py-2.5 text-left transition ${openMaterialId === category.id ? "bg-pink-50/70" : "hover:bg-pink-50/30"}`} aria-expanded={openMaterialId === category.id} aria-controls={`maria-material-${category.id}`}>
              <div className="flex gap-2.5"><div className="rounded-lg bg-pink-50 p-1.5 text-pink-600"><Layers3 className="h-4 w-4" /></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="text-xs font-extrabold text-slate-950">{category.title}</p><ChevronDown className={`h-3.5 w-3.5 shrink-0 text-slate-500 transition-transform ${openMaterialId === category.id ? "rotate-180" : ""}`} /></div><p className="mt-0.5 max-h-7 overflow-hidden text-[11px] leading-3.5 text-slate-600">{category.items.filter((item) => item.isActive).map((item) => item.title.replace(" — Perfurada", "")).join(", ")}</p></div></div>
            </button>
            {openMaterialId === category.id && <div id={`maria-material-${category.id}`} className="border-t border-pink-100 bg-pink-50/30 p-3"><p className="mb-2 text-xs font-extrabold text-pink-700">{category.title}</p><div className="space-y-2">{category.items.filter((item) => item.isActive).map((item) => <button key={item.id} type="button" onClick={() => setSelectedFinish(item)} className="flex w-full items-start gap-2 rounded-lg bg-white p-2 text-left text-xs text-slate-600 ring-1 ring-pink-100 hover:ring-pink-300"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-pink-600" /><span><strong className="text-slate-900">{item.title}.</strong> {item.description}</span></button>)}</div></div>}
          </div>)}
        </div>
        <GuideTip text={materialSection?.tip ?? "Escolha o material conforme o projeto, a instalação e o ambiente de uso."} />
      </article>

      <article id="maria-guide-acabamento" className={`mg-panel flex min-h-[450px] flex-col overflow-hidden rounded-xl border bg-white transition-shadow ${highlightedId === "acabamento" ? "border-pink-300 shadow-md" : "border-slate-200"}`}><header className="mg-panel-header flex items-start gap-3 px-4 py-4"><div className="rounded-lg bg-pink-50 p-2 text-pink-600"><GuideIcon name="crop" className="h-7 w-7" /></div><div><h3 className="font-extrabold text-slate-950">Acabamentos disponíveis</h3><p className="mt-1 text-xs leading-5 text-slate-600">{finishSection?.description}</p></div></header><div className="grid grid-cols-3 border-y border-slate-100">{finishRows.map((row, rowIndex) => { const selectedFinishInRow = row.find((item) => item.id === selectedFinish?.id); return <div key={`finish-row-${rowIndex}`} className="contents">{row.map((item) => <button key={item.id} type="button" onClick={() => setSelectedFinish(selectedFinish?.id === item.id ? null : item)} className={`min-h-[82px] border-b border-r border-slate-100 p-2 text-center transition hover:bg-pink-50/40 ${selectedFinish?.id === item.id ? "bg-pink-50" : "bg-white"}`} aria-pressed={selectedFinish?.id === item.id}><span className="mx-auto flex h-9 w-9 items-center justify-center"><FinishSymbol id={item.id} /></span><span className="mt-1 block text-[9px] font-extrabold leading-3 text-slate-900">{item.title}</span></button>)}{selectedFinishInRow && <div className="col-span-3 border-b border-pink-200 bg-pink-50 px-3 py-3 text-xs leading-5 text-slate-600" aria-live="polite"><span className="font-extrabold text-pink-700">{selectedFinishInRow.title}:</span><p className="mt-1">{selectedFinishInRow.description}</p></div>}</div>; })}</div><GuideTip text={finishSection?.tip ?? "O acabamento adequado valoriza e protege o material conforme o projeto."} /></article>

      <article id="maria-guide-entrega" className={`mg-panel flex min-h-[450px] flex-col overflow-hidden rounded-xl border bg-white transition-shadow ${highlightedId === "entrega" ? "border-pink-300 shadow-md" : "border-slate-200"}`}><header className="mg-panel-header flex items-start gap-3 px-4 py-4"><div className="rounded-lg bg-pink-50 p-2 text-pink-600"><GuideIcon name="truck" className="h-7 w-7" /></div><div><h3 className="font-extrabold text-slate-950">Entrega e retirada</h3><p className="mt-1 text-xs leading-5 text-slate-600">{deliverySection?.description}</p></div></header><div className="border-y border-slate-100">{deliveryItems.map((item, index) => <ItemRow key={item.id} item={item} icon={index === 0 ? "sparkles" : index === 1 ? "truck" : index === 2 ? "package" : index === 3 ? "printer" : "layers"} isOpen={openDeliveryId === item.id} onClick={() => setOpenDeliveryId(openDeliveryId === item.id ? null : item.id)} />)}</div><GuideTip text={deliverySection?.tip ?? "Consulte as condições disponíveis durante a configuração do pedido."} /></article>
    </div>

    <footer className="mt-4 flex flex-col gap-4 rounded-xl border border-pink-200 bg-white px-4 py-4 lg:grid lg:grid-cols-[minmax(0,1fr)_3rem] lg:items-center" style={{ paddingTop: "3px" }}><div className="flex min-w-0 items-start gap-3"><CircleHelp className="mt-0.5 h-5 w-5 shrink-0 text-pink-600" /><div><p className="font-extrabold text-slate-950">{guide.bottomNoteTitle}</p><p className="mt-1 text-xs leading-5 text-slate-600">{guide.bottomNote}</p></div></div><div className="flex w-full justify-center lg:w-12 lg:justify-end"><a href="/documentos/normas-envio-arte" aria-label="Ver normas para envio da arte" title="Ver normas para envio da arte" className={`${HOME_SECONDARY_ACTION_CLASS} h-9 min-w-[9.5rem] shrink-0 gap-1 px-3 text-[10px] lg:h-8 lg:w-12 lg:min-w-0 lg:p-0`}><FileText className="h-3 w-3 shrink-0" /><span className="whitespace-nowrap font-semibold lg:sr-only">Ver normas para envio de arte</span></a></div></footer>
  </section>;
}
