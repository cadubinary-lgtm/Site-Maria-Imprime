...
export function PublicProductCard({ product, priceAudience = "final" }: { product: ProductCardData; priceAudience?: ProductPriceAudience }) {
  ...
  return (
    ...
    <Card className="overflow-hidden border border-gray-200 bg-white p-0 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
      ...
      <CardContent className="product-card-content relative -mt-3 px-4 pb-4 pt-0">
        ...
        <div className={`mt-1.5 grid gap-2 ${isReseller ? "grid-cols-1" : "grid-cols-2"}`}>
          <div className="min-w-0">
            <p className="text-[7px] font-bold uppercase tracking-wide text-emerald-700">{isReseller ? "Preço revendedor" : "A partir de"}</p>
          ...
          {!isReseller && (
            <div className="relative z-10 min-w-0 border-l border-gray-200 bg-white pl-2">
              <p className="text-[7px] font-bold uppercase tracking-wide text-gray-400">A partir de</p>
        ...
        <span className="product-card-action mx-auto mt-2 flex h-4 w-1/2 items-center justify-center rounded-full bg-pink-600 px-1 text-[9px] font-semibold text-white shadow-sm transition-all hover:bg-pink-700 hover:shadow-md">
          Ver opções
        </span>
      </CardContent>
    </Card>
