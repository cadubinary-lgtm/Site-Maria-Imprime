...
export function ProductTagBadges({ tags, tagPosition }: ProductTagBadgesProps) {
  ...
  return (
    <div className={`absolute flex flex-col gap-1 z-10 ${posClass}`}>
      {parsed.map((tag) => (
        <span
          key={tag}
          className="product-tag-badge inline-flex items-center gap-1 bg-black text-white text-xs font-semibold px-2.5 py-1 rounded-full shadow-sm whitespace-nowrap"
        >
