...
import { Download, Mail, Plus, Printer, ReceiptText, Search } from "lucide-react";
...
export default function FinanceiroRecibos() {
  ...
  const downloadReceiptPdf = async (receiptId: number) => {
    ...
    try {
      const receiptData = await utils.financeiro.getRecibo.fetch({ receiptId });
      ...
      toast.success("PDF do recibo baixado", { position: "top-right", duration: 3500, id: `receipt-list-pdf-${receiptId}` });
    } catch (error) {
      console.error("Erro ao gerar PDF do recibo", error);
      toast.error("Não foi possível gerar o PDF do recibo.", { position: "top-right", duration: 3500, id: `receipt-list-pdf-${receiptId}` });
    } finally {
  ...
  return <AdminLayout><div className="financeiro-recibos space-y-6 p-6">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => { refetch(); refetchStandalone(); }} className="border-pink-200 text-pink-700 hover:bg-pink-50">Atualizar</Button><Button type="button" onClick={() => setLocation("/admin/financeiro/recibos/avulso/novo")} className="bg-pink-600 hover:bg-pink-700"><Plus className="mr-1 h-4 w-4" aria-hidden="true" />Criar recibo</Button></div>
    </div>
