...
import { ArrowLeft, ArrowRight, ImagePlus, Link2, Loader2, Pencil, Plus, Trash2, Upload } from "lucide-react";
...
const EMPTY_DRAFT: SlideDraft = { imageUrl: "", imageKey: "", segmentId: "" };
...
export default function AdminHomeCarousel() {
  ...
  const slides = useMemo(() => (fetchedSlides ?? []) as CarouselSlide[], [fetchedSlides]);
  ...
  const handleImageUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setDraft((current) => ({ ...current, imageUrl: payload.url, imageKey: payload.key ?? "" }));
      toast.success("Imagem carregada", { description: "Agora selecione o segmento de destino e salve." });
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      const values = { imageUrl: draft.imageUrl, imageKey: draft.imageKey || undefined, segmentId };
      ...
      toast.success(draft.id ? "Imagem do carrossel atualizada" : "Imagem adicionada ao carrossel", {
        description: "O botão Ver opções direcionará ao segmento escolhido.",
      });
    } catch (error) {
              ...
              <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                {slides.map((slide, index) => (
                  <article key={slide.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div className="relative aspect-[16/9] bg-slate-100"><img src={slide.imageUrl} alt={`Imagem ${index + 1} do carrossel para ${slide.segmentName}`} className="h-full w-full object-cover" /><span className="absolute left-3 top-3 rounded-full bg-slate-950/75 px-2.5 py-1 text-xs font-bold text-white">{index + 1}ª imagem</span><span className="absolute bottom-3 left-3 rounded-lg bg-pink-600 px-3 py-1.5 text-xs font-bold text-white shadow-sm">Ver opções</span></div>
