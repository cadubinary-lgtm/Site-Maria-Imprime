...
export default function AdminVariationsCv() {
  ...
  const { data: offsetTypes = [], refetch } = trpc.variationsCv.getGlobal.useQuery();

  // ── Mutations ─────────────────────────────────────────────────────────────
  const createTypeMutation = trpc.variationsCv.createType.useMutation();
  const updateTypeMutation = trpc.variationsCv.updateType.useMutation();
  const deleteTypeMutation = trpc.variationsCv.deleteType.useMutation();
  const createOptionMutation = trpc.variationsCv.createOption.useMutation();
    ...
    <AdminLayout>
      <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Variações Comunicação Visual</h1>
            <p className="text-sm text-gray-500 mt-1">
              Gerencie tipos de variação exclusivos para produtos de Comunicação Visual.
              Estas variações são <strong>completamente independentes</strong> das variações de Offset.
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => navigate("/admin/variacoesoffset")}>
              Variações Offset
            </Button>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Adicionar Novo Tipo de Variação Comunicação Visual</h2>
