        ...
        <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); if (form.newPassword && form.newPassword !== form.confirmPassword) { toast.error("As senhas não coincidem."); return; } if (customer) { const { confirmPassword, ...payload } = form; updateCustomer.mutate({ customerId: customer.id, ...payload }); } }}>
          <section className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <label className="grid gap-1.5 text-sm font-medium text-gray-700"><span>Nova senha (opcional)</span><Input type="password" name="customer-new-password" autoComplete="new-password" placeholder="Deixe em branco para manter a atual" value={form.newPassword} onChange={(event) => setForm({ ...form, newPassword: event.target.value })} /></label>
              <label className="grid gap-1.5 text-sm font-medium text-gray-700"><span>Confirmar nova senha</span><Input type="password" name="customer-confirm-new-password" autoComplete="new-password" placeholder="Repita a nova senha" value={form.confirmPassword} onChange={(event) => setForm({ ...form, confirmPassword: event.target.value })} /></label>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {showPartnerForm && (
            <Card className="mb-6 border-pink-100"><CardHeader><CardTitle className="text-base text-gray-900">Novo cliente</CardTitle><p className="text-sm text-gray-500">Escolha o tipo antes de criar. O cadastro será refletido na lista unificada e nos indicadores de clientes.</p></CardHeader><CardContent><form onSubmit={handleCreateCustomer} className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div className="grid gap-3 md:col-span-2 md:grid-cols-2"><div><label htmlFor="creation-client-type" className="mb-1.5 block text-sm font-medium text-gray-700">Tipo de cliente *</label><Select value={creationClientType} onValueChange={(value: "site" | "balcao" | "revendedor" | "agencia") => setCreationClientType(value)} disabled={Boolean(forcedClientType)}><SelectTrigger id="creation-client-type" aria-label="Tipo de cliente para o novo cadastro"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="site">Cliente site</SelectItem><SelectItem value="balcao">Cliente balcão</SelectItem><SelectItem value="revendedor">Revendedor</SelectItem><SelectItem value="agencia">Agência</SelectItem></SelectContent></Select><p className="mt-1 text-xs text-gray-500">{isBalcaoCreation ? "Cliente de balcão não recebe acesso ao site; os dados ficam disponíveis no CRM." : "Este tipo terá acesso ao site e receberá um e-mail para definir a senha."}</p></div><label className="grid content-start gap-1.5 text-sm font-medium text-gray-700"><span>Tabela de preços</span><Select value={partnerForm.priceTier} onValueChange={(value: "final" | "reseller") => setPartnerForm({ ...partnerForm, priceTier: value })}><SelectTrigger aria-label="Tabela de preços do novo cliente"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="final">Cliente final</SelectItem><SelectItem value="reseller">Revendedor</SelectItem></SelectContent></Select></label></div>
