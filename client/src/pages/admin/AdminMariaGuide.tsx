  ...
  return <AdminLayout>
    <div className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <section className="flex flex-col gap-5 rounded-2xl border border-pink-100 bg-gradient-to-r from-white via-pink-50 to-white p-6 shadow-sm xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" className="border-pink-200 text-pink-700 hover:bg-pink-50" onClick={() => setIsPreviewVisible((value) => !value)}><Eye className="mr-2 h-4 w-4" />{isPreviewVisible ? "Fechar prévia" : "Ver prévia"}</Button><Button type="button" variant="outline" onClick={() => handleSaveDraft()} disabled={saveDraft.isPending}>{saveDraft.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Salvar rascunho</Button><Button type="button" className="bg-pink-600 hover:bg-pink-700" onClick={handlePublish} disabled={saveDraft.isPending || publishGuide.isPending}>{publishGuide.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />}Publicar alterações</Button></div>
      </section>
      ...
      {isLoading ? <div className="flex min-h-64 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-pink-600" /></div> : <>
        ...
        <div className="grid gap-6 xl:grid-cols-[250px_minmax(0,1fr)]"><Card className="h-fit"><CardHeader><CardTitle className="text-base">Cards do guia</CardTitle><CardDescription>Ative, desative e escolha a área que deseja editar.</CardDescription></CardHeader><CardContent className="space-y-2">{guide.sections.map((section) => <button key={section.id} type="button" onClick={() => setActiveSectionId(section.id)} className={`w-full rounded-xl border px-4 py-3 text-left transition ${section.id === activeSection.id ? "border-pink-300 bg-pink-50 text-pink-700" : "border-slate-200 bg-white text-slate-700 hover:border-pink-200"}`}><span className="block text-sm font-extrabold">{section.title}</span><span className="mt-1 block text-xs leading-4 text-slate-500">{section.subtitle}</span></button>)}</CardContent></Card>
