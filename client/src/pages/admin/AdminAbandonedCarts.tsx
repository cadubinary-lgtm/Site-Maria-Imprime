        ...
        <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="flex items-center gap-3 text-3xl font-bold text-gray-900">
              <ShoppingCart className="h-8 w-8 text-pink-600" aria-hidden="true" />
              Carrinho Abandonado
            </h1>
          ...
          <div className="flex gap-2">
            ...
            >
              {cleanupMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" aria-hidden="true" /> : <Trash2 className="mr-2 h-4 w-4" aria-hidden="true" />}
              Limpar expirados
            </Button>
          </div>
        </div>

        <div className="mb-6 grid gap-4 md:grid-cols-3" aria-label="Indicadores de carrinhos abandonados" aria-live="polite">
        ...
        <Card>
          <CardContent className="space-y-4 pt-6">
            <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_170px_170px]">
              ...
              <label htmlFor="abandoned-cart-search" className="sr-only">Buscar carrinhos abandonados</label>
              ...
              <div><label htmlFor="abandoned-cart-start-date" className="sr-only">Data inicial</label><Input id="abandoned-cart-start-date" type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></div>
              <div><label htmlFor="abandoned-cart-end-date" className="sr-only">Data final</label><Input id="abandoned-cart-end-date" type="date" value={endDate} onChange={(event) => setEndDate(event.target.value)} /></div>
            </div>

            {isLoading ? (
              <div className="flex justify-center py-16"><Loader2 className="h-7 w-7 animate-spin text-pink-600" aria-label="Carregando carrinhos abandonados" /></div>
            ) : filteredCarts.length === 0 ? (
              <div className="py-16 text-center">
                <ShoppingCart className="mx-auto mb-3 h-12 w-12 text-gray-200" aria-hidden="true" />
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[760px] text-sm">
                  <thead className="border-b bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
                    <tr><th scope="col" className="px-4 py-3 font-medium">Carrinho</th><th scope="col" className="px-4 py-3 font-medium">Produtos</th><th scope="col" className="px-4 py-3 font-medium">Itens</th><th scope="col" className="px-4 py-3 font-medium">Valor</th><th scope="col" className="px-4 py-3 font-medium">Última atividade</th><th scope="col" className="px-4 py-3 font-medium">Lembrete</th><th scope="col" className="px-4 py-3 font-medium">Retenção</th><th scope="col" className="px-4 py-3 text-right font-medium">Ações</th></tr>
                  </thead>
                    ...
                    {filteredCarts.map((cart) => (
                      <tr key={cart.cartKey} className="hover:bg-gray-50/70">
                        ...
                        <td className="px-4 py-4">
                          {cart.emailReminderSentAt ? <Badge className="gap-1 bg-emerald-100 text-emerald-700 hover:bg-emerald-100"><CheckCircle2 className="h-3 w-3" aria-hidden="true" />E-mail enviado</Badge>
                            : cart.whatsappReminderOpenedAt ? <Badge className="gap-1 bg-emerald-50 text-emerald-700 hover:bg-emerald-50"><MessageCircle className="h-3 w-3" aria-hidden="true" />WhatsApp preparado</Badge>
                  ...
                  return (
                    <div key={item.id} className="flex gap-4 rounded-xl border border-gray-100 p-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {item.artFileUrl && <a href={item.artFileUrl} target="_blank" rel="noopener noreferrer" className="mt-2 inline-block text-xs font-medium text-pink-600 hover:underline">Ver arquivo de arte<span className="sr-only">, em nova aba</span></a>}
                      </div>
                    </div>
                  );
                })}
