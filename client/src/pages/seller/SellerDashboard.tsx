...
export default function SellerDashboard() {
  ...
  const cards = [
    ...
    { label: "Orçamentos emitidos", value: String(quotations?.rows.length ?? 0), icon: FileText, tone: "text-pink-600 bg-pink-50" },
  ];
  return <SellerLayout title={`Olá, ${profile?.name?.split(" ")[0] ?? "vendedor"}`} description="Acompanhe sua carteira comercial e suas comissões.">
    ...
    <section className="mt-6 grid gap-6 lg:grid-cols-2">
      ...
      <Card><CardHeader className="flex-row items-center justify-between"><CardTitle className="text-base">Orçamentos recentes</CardTitle><Button asChild variant="link" size="sm"><Link href="/vendedor/orcamentos">Ver todos</Link></Button></CardHeader><CardContent className="space-y-3">{quotations?.rows.slice(0, 5).map((quote: any) => <div key={quote.id} className="flex items-center justify-between border-b border-slate-100 pb-3 text-sm last:border-0"><div><p className="font-medium">{quote.quotationNumber}</p><p className="text-slate-500">{quote.clientName ?? "Cliente"}</p></div><span className="font-medium">{currency(quote.total)}</span></div>) ?? <p className="text-sm text-slate-500">Nenhum orçamento registrado.</p>}</CardContent></Card>
    </section>
  </SellerLayout>;
}
