    ...
    rows = await db.execute(
      sql`
        SELECT 
          ...
          ci.selectedAttributes, ci.customDimensions, ci.priceAtCart, ci.pixPriceAtCart, ci.cardPriceAtCart, ci.selectedPaymentMethod,
...
export async function addToCart(data: {
  ...
  priceAtCart: number;
  pixPriceAtCart?: number;
  cardPriceAtCart?: number;
  selectedPaymentMethod?: "pix" | "cartao";
  artFileUrl?: string;
  artFileUrls?: string;
  notes?: string;
  ...
  const result = await db.execute(
    sql`
      INSERT INTO cartItems (
        ...
        priceAtCart, pixPriceAtCart, cardPriceAtCart, selectedPaymentMethod, artFileUrl, artFileUrls, notes, shippingMethod, shippingPrice, shippingLabel,
      ...
      VALUES (
        ...
        ${data.priceAtCart}, ${data.pixPriceAtCart ?? data.priceAtCart}, ${data.cardPriceAtCart ?? data.priceAtCart}, ${data.selectedPaymentMethod ?? "pix"}, ${data.artFileUrl ?? null}, ${data.artFileUrls ?? null}, ${data.notes ?? null},
...
export async function clearDeletedAbandonedCartHistory() {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.execute(sql`DELETE FROM deletedAbandonedCarts`) as any;
...
export async function createOrderFromCart(data: {
  ...
  salesOwnerType?: "seller" | "admin" | null;
  ...
  try {
    const orderResult = await db.execute(
      sql`
        INSERT INTO orders (
          clientId, userId, customerId, sellerId, sellerName, salesOwnerType, salesOwnerName, orderNumber, status, totalPrice, paymentStatus, notes,
          ...
          ${data.clientId}, ${data.userId}, ${data.customerId ?? null}, ${data.sellerId ?? null}, ${data.sellerName ?? null}, ${data.salesOwnerType ?? null}, ${data.salesOwnerName ?? null}, ${data.orderNumber},
