...
describe("cards públicos informativos", () => {
  it("mantém a composição compacta e só exibe urgência de m² quando ela estiver configurada", () => {
    ...
    expect(publicCardSource).not.toContain("sameDayUrgency");
    expect(publicCardSource).not.toContain("deliveryOptions.getByProduct");
    expect(publicCardSource).not.toContain("Produção no mesmo dia · taxa de urgência");
    expect(publicCardSource).toContain("cardDescriptionLines.length > 0 &&");
    expect(publicCardSource).toContain("flex-col gap-0.5");
    expect(publicCardSource).toContain('className="truncate"');
    expect(publicCardSource).not.toContain('className="product-card-urgency-content min-w-0 break-words"');
    ...
    expect(tagBadgeSource).toContain("w-2/5");
    expect(tagBadgeSource).toContain("product-tag-badge flex h-3 w-full");
